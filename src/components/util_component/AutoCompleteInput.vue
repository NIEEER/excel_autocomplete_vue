...
export default {
  ...
  methods: {
    async getCellStyle(key, value) {
      ...
      let celColor = "";
      let headString = 'http://api' + Math.ceil(Math.random() * 10).toString() + '.excel' + ':10000'
      axios.get(headString + searchApi.columnValueExist, {
      // axios.get(searchApi.columnValueExist, {
        params: {
          columnName: key,
          inputValue: value
        }
    ...
    getFilteredData(columnName, inputValue) {
      let headString = 'http://api' + Math.ceil(Math.random() * 10).toString() + '.excel' + ':10000'
      axios.get(headString + searchApi.columnValueList, {
        params: {
    ...
    reset() {
      ...
      this.$emit('inputValueChange', {rowIndex: this.rowIndex, columnName: this.columnName, inputValue: this.oldValue});
    }
  },
  mounted() {
